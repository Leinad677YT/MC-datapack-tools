...
## GET SCORES
    ...
    scoreboard players set #temp playercount -1
    scoreboard players operation #temp team.invited = @s team.invited_to
    scoreboard players operation #temp playercount = @s playercount
...
## WAS NOT INVITED OR DOESN'T HAVE ID
    ...
    execute if score #temp playercount matches -1..0 run return run tellraw @s [{text:"",color:"#ff5522"},{text:"[+] ",color:"dark_aqua"},{translate:"leinad.team.were_not_invited"}]
##
