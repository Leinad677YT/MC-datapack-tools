## GET UUID + id DATA
    $data modify storage leinad_temp:player is_new.from_uuid set from storage player:map $(list)[{uuid:$(uuid)}]
    $data modify storage leinad_temp:player is_new.from_id set from storage player:map $(list)[{id:$(id)}]
##
...
## PREPARE COMPARE id VS UUID ~ id
    execute store result score #bool player_id run data get storage leinad_temp:player is_new.from_uuid.id
##

## CASE 3: IS SELF    (UUID ~ id)
    # doesn't do anything afterwards, it's Pepito777 :D
    execute if score #bool player_id = @s player_id run return 3
##

## CASE 4: CHANGED TO OCCUPIED NICKNAME    (UUID !~ id)
    # get's old id and sets it on this nickname to access data
    $execute store result score @s player_id run data get storage player:map $(list)[{uuid:$(uuid)}].id
    return 4
    # 
##
