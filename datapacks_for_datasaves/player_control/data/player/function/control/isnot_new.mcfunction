...
## NEW NICKNAME?
    execute unless score @s player_id matches -2147483648..2147483647 run scoreboard players set @s player_id -2147483648
...
## UUID ~ NICKNAME?
    ...
    ## GET LIST AND UUID 
        ...
        execute store result storage leinad_temp:player macro.id int 1 run scoreboard players get @s player_id
    ##

    ## GET (UUID ~ id) SITUATION
        execute store result score #bool player_id run function player:macro/check_id with storage leinad_temp:player macro
    ##
##
...
## CASE 3: IS SELF    (UUID ~ id)
 ...
 ## CASE 4: CHANGED TO OCCUPIED NICKNAME  (fixed inside player:macro/check_id)
    ## (4) save data from old owner
        execute if score #bool player_id matches 4 run function player:db/store_old_owner with storage leinad_temp:player is_new.from_id
    ##
    ## (4) clone own data to new name
        execute store result storage leinad_temp:player is_new.from_uuid.id int 1 run scoreboard players get @s player_id
        data modify storage leinad_temp:player is_new.from_uuid.newname set from storage stringlib:output to_lowercase 
        execute if score #bool player_id matches 4 run function player:db/store_new_old_owner with storage leinad_temp:player is_new.from_uuid
        execute if score #bool player_id matches 4 run function player:macro/update_uuid_map with storage leinad_temp:player macro
    ##

    execute if score #bool player_id matches 3 run return 1
    execute if score #bool player_id matches 4 run return 2
##
...
## CASE 1: NEW ACCOUNT
 ...
 ## CASE 2: NEW UUID, OLD id
    ## (2) save old data
        execute if score #bool player_id matches 2 run function player:db/store_old_owner with storage leinad_temp:player is_new.from_id
    ##

    ## SET NEW {UUID-id MAP + DATA}
        execute store result score @s player_id run scoreboard players add #newID player_id 1
    ...
    ## SET NEW DATA
        function player:db/new_player with storage leinad_temp:player macro
    ##
    return fail
##
